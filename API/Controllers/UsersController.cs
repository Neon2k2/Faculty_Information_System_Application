...
namespace Faculty_Information_System_Application.Controllers
{
    ...
    public class UsersController : ControllerBase
    {
        ...
        private readonly IJWTManagerRepository _repository1;
        ...
        [HttpGet]
        [Route("{username}/{password}/{roleLookupId}")]
        public IActionResult ValidateUser(string username, string password, int roleLookupId)
        {
            var jwtToken = _repository1.Authenticate(username, password, roleLookupId);
            if (jwtToken != null)
            {
                return Ok(jwtToken);
            }
